...
class CentralSyncService
{
    ...
    private function send(string $endpoint, array $payload): bool
    {
        ...
        $url = rtrim((string) config('services.mci_central.url'), '/');
        $token = config('services.mci_central.token');

        if ($url === '' || ! $token) {
            Log::warning('MCI central sync skipped: configuration incomplete.', ['endpoint' => $endpoint]);
            return false;
        }
        ...
        try {
            $response = Http::acceptJson()
                ...
                ->timeout((int) config('services.mci_central.timeout', 10))
