...
<title>Trial Applications | C-Net Web Services</title>
<style>
*{box-sizing:border-box}body{margin:0;background:#f3f7fb;color:#26384a;font-family:Arial,sans-serif}
header{background:#07223d;color:#fff;padding:18px}.container{width:min(1250px,96%);margin:auto}
.top{display:flex;justify-content:space-between}.top a{color:#fff;text-decoration:none;border:1px solid #ffffff55;padding:8px 12px;border-radius:7px}
...
.heading h2{margin:0;color:#07223d}.badge{padding:6px 11px;border-radius:20px;background:#eaf2fa;font-weight:700}
.details{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}.details strong{display:block;color:#07223d}
.actions{display:grid;grid-template-columns:1fr 1fr 1fr;gap:15px;margin-top:20px;padding-top:17px;border-top:1px solid #e4ebf2}
...
.upgrade{background:#8a4b08;color:#fff;border:0;font-weight:700}.trial-link{color:#0756a3;font-weight:700}
...
<section class="application">
    <div class="heading">
        <div><h2>{{ $application->business_name }}</h2><span>{{ $application->category }} • {{ $application->owner_name }}</span></div>
        <span class="badge">{{ strtoupper($application->status) }}</span>
    </div>

    <div class="details">
        <div><strong>Contact</strong>{{ $application->phone }}<br>{{ $application->email }}</div>
        <div><strong>Website Name</strong>{{ $application->desired_slug }}</div>
        <div><strong>Valid Until</strong>{{ $application->expires_at ?: 'Not started' }}</div>
        <div><strong>Selected Plan</strong>{{ $application->selected_plan_title ?: 'Not upgraded' }}</div>
    </div>

    <p>{{ $application->about_business }}</p>

    @if($application->trial_url)
        <a class="trial-link" target="_blank" href="{{ $application->trial_url }}">Open Trial Website ↗</a>
    @endif
    ...
    <div class="actions">
        ...
        <form method="POST" action="{{ route('admin.trials.extend',$application->id) }}">
            ...
            <p>Add 7 more days to the current validity.</p>
...
@empty
<div class="application">No trial applications received yet.</div>
@endforelse
</main>
</body>
</html>
